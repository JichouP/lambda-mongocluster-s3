...
const dumpUris = process.env.MONGODUMP_URIS.split(',');
const bucketName = process.env.S3_BUCKET;
const s3bucket = new AWS.S3({ params: { Bucket: bucketName } });
const s3StorageClass = process.env.S3_STORAGE_CLASS || 'STANDARD';
const zipFilenames = process.env.ZIP_FILENAME.split(',') || ['mongodb_backup'];
const dateFormat = process.env.DATE_FORMAT || 'YYYYMMDD_HHmmss';

const entries = dumpUris.map((dumpUri, i) => ({
  dumpUri,
  zipFilename: zipFilenames[i] || 'mongodb_backup',
}));

module.exports.handler = function (_event, _context, _cb) {
  for (const { dumpUri, zipFilename } of entries) {
    ...
    exec(
      `mongodump --uri ${dumpUri} --out ${folderName}`,
